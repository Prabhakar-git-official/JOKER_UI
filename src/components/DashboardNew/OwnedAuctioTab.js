  ...
  useEffect(()=>{dbcallProfile()},[])        
    ...
    const setpricedb=async(b)=>{      
      ...
      let transaction1 = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
       ...
       appArg.push(algosdk.encodeUint64(parseInt(a)))
       appArg.push(algosdk.encodeUint64(parseInt(time)))
      ...
      const groupID = algosdk.computeGroupID([transaction2,transaction1,transaction3, transaction4]);
      const txs = [transaction2,transaction1, transaction3, transaction4];
      txs[0].group = groupID;
      txs[1].group = groupID;
      txs[2].group = groupID;
      txs[3].group = groupID;              
      ...
      {            
      const signedTx1 = await myAlgoWallet.signTransaction([txs[1].toByte(),txs[3].toByte()]);
      const signedTx2 = algosdk.signLogicSigTransaction(txs[0], lsig);
      const signedTx3 = algosdk.signLogicSigTransaction(txs[2], lsig);                
      response = await algodClient.sendRawTransaction([signedTx2.blob,signedTx1[0].blob, signedTx3.blob, signedTx1[1].blob]).do();      
      ...
      toast.success(toastDiv(id));
          ...
          const signedTx3 = algosdk.signLogicSigTransaction(txns[0], lsig);
        ...
        fireDb.database().ref(`imageSaleAlgosAuction/${localStorage.getItem('walletAddress')}`).child(b.keyId).set({
            ...
            fireDb.database().ref(`imageAuctionbuy/${localStorage.getItem('walletAddress')}`).child(b.keyId).remove();
