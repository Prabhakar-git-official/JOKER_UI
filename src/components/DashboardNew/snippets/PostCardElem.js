...
import { Modal, Button, ProgressBar, Form, InputGroup, Card  } from 'react-bootstrap';
...
const PostCardElem = () => {
    ...
    const [whiteAddress,setToaddress] = useState("");
    ...
    let appID_global = launchpadDetails['app2']['appID'];
    let escrow_global = "LMCGCWB7LOFIQBIKO663W4OOOQQCNWQGU23HCMLYXX3S35OXS47XLXLTXQ";
    let elementID_global = launchpadDetails['app2']['elemAssetID'];
...
// const algodClientGet = new algosdk.Algodv2(token, baseServer, port);
  ...
  return
          ...
          let program = new Uint8Array(Buffer.from(launchpadDetails['app2']['escrow'], "base64"));          
...
{
      try {
        let appById = await indexClient.lookupApplications(appID_global).do();
...
const whiteList = async () =>
{
    try {
        //   const accounts = await myAlgoWallet.connect();
          ...
          let appArgs1 = [];
          appArgs1.push(new Uint8Array(Buffer.from("whiteList")));
        ...
        //   let recv_escrow = escrow;
          ...
          let transaction1 = algosdk.makeApplicationNoOpTxnFromObject({
            from:sender, 
            suggestedParams: params, 
            appIndex: launchpadDetails['app1']['appID'], 
            appArgs: appArgs1
          })                    
...
{/* <><ToastContainer position='top-center' draggable = {false} transition={Zoom} autoClose={8000} closeOnClick = {false}/></> */}
            ...
            <Card className='card-dash border-0 d-block'>
                ...
                <div className="post-card-title mb-2 w-100 d-flex align-items-center">
                    ...
                    <div>
                        <h6 className='m-0'>WhiteList Admin</h6>
                ...
                <div className="post-card-footer">
                    <div>
                    <Form.Control type="text" placeholder="Enter WhiteListed Address" required onChange={event => setToaddress(event.target.value)} />
                    {/* <label> WhiteListed Address : </label> 
                    <input
                      id="addressid"
                      type='text'
                      ...
                      name="toaddress"
                    /> */}<br/>
                    <Button className='w-100' onClick={whiteList}>Make WhiteList</Button>
                ...
                <Modal.Header className='align-items-start' closeButton>
                    <div className="d-flex flex-wrap align-items-start justify-content-between">
                        <div className="d-flex align-items-center flex-wrap modal-head">
                            ...
                            {appOpt === false ? <><Button variant="blue" className='py-1' onClick={()=>AppOptIn()} style={{textTransform:"capitalize"}}>app opt-in</Button><p style={{color:"red"}}>(Please Opt-In App to Participate)</p></> : <></>}
