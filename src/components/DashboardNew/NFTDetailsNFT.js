  ...
  listenMMAccount();
    ...
    const BuyNFT =async()=>{
        ...
        {            
            ...
            params.fee = 1000;
            params.flatFee = true;              
            let dataopreplace = dataNFTcreate.replaceAll("AppID",configfile['NFTappId']).replaceAll("AssId",parseInt(location.state.allData.Assetid))            
        ...
        //db change here
          ...
          {
            firebase.auth().signInAnonymously().then((response)=>{                              
                fireDb.database().ref(`imageSaleAlgosNFT/${location.state.allData.ownerAddress}`).child(location.state.allData.keyId).remove().then(()=>{
                fireDb.database().ref(`imageNNFTbuy/${localStorage.getItem("walletAddress")}`).child(location.state.allData.keyId).set({
    ...
    const BuyOptNFT=async()=>{
        ...
        {            
            ...
            try {                
            const params = await algodClient.getTransactionParams().do();                            
            let dataelem = dataNFTcreate.replaceAll("AppID",configfile['NFTappId']).replaceAll("AssId",parseInt(location.state.allData.Assetid))                        
