...
const OwnedFrctionTab =({x})=>{
    ...
    const setpricedb=async(b)=>{        
      ...
      let transaction1 = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
       ...
       appArg.push(new Uint8Array(Buffer.from("CreateListingcopy")));       
       appArg.push(algosdk.encodeUint64(parseFloat(amountmul)))  
       appArg.push(algosdk.encodeUint64(parseFloat(getduplicateAssetValue)))
       appArg.push(algosdk.encodeUint64(parseFloat((getduplicateAssetValue*amountmul))))
       const transaction2 = algosdk.makeApplicationNoOpTxnFromObject({
           from: recv_escrow, 
           appIndex: index,
           appArgs: appArg,
        ...
        const transaction4 = algosdk.makeAssetTransferTxnWithSuggestedParamsFromObject({
          ...
          assetIndex: parseInt(b.Assetid),
          note: undefined,
          amount: parseInt(getduplicateAssetValue),
          suggestedParams: params
        });
      ...
      toast.success(toastDiv(id));      
        ...
        fireDb.auth().signInAnonymously().then((responses)=>{                     
            fireDb.database().ref(`imageSaleAlgosFractional/${localStorage.getItem('walletAddress')}`).child(b.keyId).set({
                ...
                NFTName:b.NFTName,userSymbol:b.userSymbol,Ipfsurl:b.Ipfsurl,ownerAddress:localStorage.getItem('walletAddress'),previousoaddress:b.previousoaddress,
                TimeStamp:dateset,NFTDescription:b.NFTDescription,HistoryAddress:b.HistoryAddress,Appid:b.Appid,valid:b.valid,
                CreatorAddress:localStorage.getItem('walletAddress'),NFTType:b.NFTType,NFTChannel:b.NFTChannel,SocialLink:b.SocialLink,NFTModel:b.NFTModel
            }).then(()=>{
            fireDb.database().ref(`imageFractiontruebuy/${localStorage.getItem('walletAddress')}`).child(b.keyId).remove();
