...
const CreateTab =({x})=>{
    ...
    const setpricedb=async(b)=>{        
      ...
      let transaction1 = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
       ...
       console.log("Apparg",amountmul +"==="+ getduplicateAssetValue +"==="+ (getduplicateAssetValue*amountmul))
      ...
      if(b.EscrowAddress === null || b.EscrowAddress === undefined || b.EscrowAddress === ""){                
        await Freezefn(lsig.address(),b.Assetid)
      }else{
        console.log("Nothing")        
      }      
      }
...
const Freezefn =async(escrow,asset)=>{
   let params = await algodClient.getTransactionParams().do();    
    ...
    let ctxn = algosdk.makeAssetConfigTxnWithSuggestedParamsFromObject(
        {  
         ...
         if(localStorage.getItem("walletName") === "myAlgoWallet"){
                const signedTx1first = await myAlgoWallet.signTransaction(ctxn.toByte());
                let responsefirst = await algodClient.sendRawTransaction(signedTx1first.blob).do();                                          
