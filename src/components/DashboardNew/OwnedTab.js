...
const OwnedTab =({x})=>{
    ...
    const setpricedb=async(b)=>{        
        ...
        if(minbalance < (961000+2000)){
            ...
            let accountInfo = await indexClient.lookupAccountByID(b.EscrowAddress).do();              
            const apps = accountInfo['account']['apps-local-state'];
            let appCount = apps['length'];
            let PriceRoyalty = "";            
            for(let j = 0; j < appCount; j++)
              {
              if(accountInfo['account']['apps-local-state'][j]['id'] === parseInt(configfile.royaltyappId))
              {                
                  let localstatecount = accountInfo['account']['apps-local-state'][j]['key-value']['length'];
                  for(let k=0;k<localstatecount;k++){                    
                    if(accountInfo['account']['apps-local-state'][j]['key-value'][k]['key'] === "Um95YWx0eWZlZQ==" ){                                            
                      PriceRoyalty = accountInfo['account']['apps-local-state'][j]['key-value'][k]['value']['uint']
                      break;
                    }
                  }                                
            ...
            if(PriceRoyalty === "0" || PriceRoyalty === 0 || parseInt(PriceRoyalty) === "0" || parseInt(PriceRoyalty) === 0){                    
        handleShowLoad()     
        ...
        let dataopreplace = datanewpriceTHiru.replaceAll("AppID",parseInt(configfile['royaltyappId'])).replaceAll("AssId",parseInt(b.Assetid))                
      ...
      toast.success(toastDiv(id));
        ...
        fireDb.auth().signInAnonymously().then((responses)=>{                     
          let dateset=new Date().toDateString();      
            fireDb.database().ref(`imageSaleAlgosRoyalty/${localStorage.getItem('walletAddress')}`).child(b.keyId).set({
                Assetid:b.Assetid,Imageurl:b.Imageurl,NFTPrice:amountmul,EscrowAddress:lsig.address(),keyId:b.keyId,
                ...
                fireDb.database().ref(`imageRoyaltybuy/${localStorage.getItem('walletAddress')}`).child(b.keyId).remove();
        ...
        //db end here                  
            ...
            toast.error("Please Wait Sometimes",{autoClose:5000});  
            handleHideLoad()              
        }
        }              
        }            
